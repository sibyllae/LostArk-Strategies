...
const FavorabilityPC: React.FC<FavorabilityCompIn> = ({
  ...
  return (
    <div className='hidden xl:grid grid-cols-2 2xl:grid-cols-3 gap-4 w-full'>
      {favorabilityData.map((record, idx) => (
        <Card
          key={idx}
          className='!bg-zinc-800/60 !border-zinc-700/50 hover:!border-rose-500/50 transition-colors flex flex-col h-full'
          title={
          ...
          <div className='space-y-3 flex-grow'>
            ...
            <div className='grid grid-cols-2 gap-2'>
              <div className='bg-zinc-900/60 rounded-lg p-2 border border-zinc-700/30'>
                <p className='text-zinc-500 text-xs mb-1'>普通</p>
                <p className='text-zinc-300 text-sm'>{record.normal || '-'}</p>
              </div>
